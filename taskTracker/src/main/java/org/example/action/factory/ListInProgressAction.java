 ...
 * @author 96361
 */
public class ListInProgressAction implements IAction {

    Gson gson;
    int targetId = 0;
    TaskStatus targetStatus = TaskStatus.TODO;

    public ListInProgressAction(Gson gson) {
        this.gson = gson;
    }
    ...
    public void performFromInput(String[] input, @Nullable String addName) {
        // task-cli list todo
        if (input.length == 3) {
            targetStatus = TaskStatus.valueOf(input[2].toUpperCase().replace("-", "_"));
        }
